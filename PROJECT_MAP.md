...
| `src/lib/dragDrop.ts` | `cellDroppableId`/`parseCellDroppableId` encode/decode an `AssignmentGrid`/`AssignmentDayView` cell's `(personId, mealType, day)` into the string id `@dnd-kit/core` droppables use. `pickedEntryDraggableId`/`parsePickedEntryDraggableId` similarly namespace a `PickedMealCard`'s draggable id by `scope` (`'desktop'`/`'mobile'`) — needed because the same entry's card is mounted twice at once (desktop sidebar + mobile "Pick meals" tab, toggled by CSS only), so each needs a distinct dnd-kit id or the two registrations collide. |
...
| `src/lib/clipboard.ts` | `copyToClipboard` — copies text via the Clipboard API when in a secure context, else falls back to the legacy `execCommand('copy')` path (needed because the Home Assistant add-on is often reached over plain HTTP, where the Clipboard API is unavailable); returns whether the copy succeeded so callers can show a manual-copy fallback otherwise. |
...
| `src/screens/PlanBuilderScreen.tsx` (+ `.module.css`) | Composition root for meal-picking and placement. Renders two structurally separate subtrees — a desktop sidebar+main layout and a mobile 2-tab ("Pick meals" / "Assign") layout — both always mounted, with visibility toggled purely by the `@media (max-width:700px)` breakpoint (no JS viewport check). Both share the same derived data/handlers and a single `DndContext` (pointer + touch sensors, the latter with a press-and-hold activation delay so a swipe still scrolls) plus a `DragOverlay` ghost card. Desktop keeps drag-and-drop onto `AssignmentGrid` (`onDragEnd` + `parseCellDroppableId`/`parsePickedEntryDraggableId`); mobile adds tap-to-place (`handleSelectEntry`/`handleTapPlace`, calling the same `assignMealToCell`) against `AssignmentDayView`, auto-switching to the "Assign" tab once a card is selected. `MealPickerPanel` and `PickedMealsPanel` are rendered once per layout so the same markup/handlers back both. |
| `src/components/MealPickerPanel.tsx` | The "Choose meals" picker extracted from `PlanBuilderScreen`: meal-type/protein filter chips plus the filtered picker list (`+ Add` calls `addPlannedMeal`). Rendered by both the desktop and mobile layouts. |
| `src/components/PickedMealsPanel.tsx` | The picked-meals list extracted from `PlanBuilderScreen`, including `PickedMealCard`. Each card is a `@dnd-kit/core` `useDraggable` (id namespaced by a required `scope: 'desktop' \| 'mobile'` prop via `pickedEntryDraggableId`, since the same entry's card mounts in both layouts at once) with a dedicated drag-handle button (so dragging doesn't compete with taps on the checkbox/number-input/Remove button) and a tap-to-select `onClick` on the card body used by the mobile tap-to-place flow. Shows the "X/Y placed" badge (`getEntryConsumedServings`/`formatServingCount`), plus the "Leftover from last week" checkbox (`setEntryLeftover`) and "Servings left" input (`setEntryServingsLeft`). |
...
| `src/components/AssignmentGrid.tsx` (+ `.module.css`) | Desktop-only Plan Builder view: Meal-type × Day drag-and-drop grid (one horizontally-scrolling sub-grid per person), rendering an `AssignmentCell` per (person, day, mealType) with `droppable` gated by `canPlaceEntryInCell`. Hidden on mobile in favor of `AssignmentDayView`. |
| `src/components/AssignmentCell.tsx` (+ `.module.css`) | One (person, day, mealType) cell, extracted so both `AssignmentGrid` (desktop, drag-and-drop) and `AssignmentDayView` (mobile, tap-to-place) share it. Takes an explicit `dropId` for its `@dnd-kit/core` `useDroppable` — callers must give each simultaneously-mounted instance of the same cell a distinct id (see `lib/dragDrop.ts`), since a disabled droppable still occupies its id in dnd-kit's shared registry. Unavailable cells (per `DayPlannerGrid`) are disabled and reject both drops and taps; a cell can hold up to `MAX_CELL_OCCUPANTS` combined items — each occupant renders its name, its derived `%` share (`getAssignmentFraction`), a +/− weight stepper (`setAssignmentWeight`, disabled when `canSetAssignmentWeight` says a change would be invalid), and a "×" to remove it (`clearAssignment`). An empty or fillable cell also accepts a click when `tapEligible` (mobile tap-to-place), firing `onTapPlace`. |
| `src/components/AssignmentDayView.tsx` (+ `.module.css`) | Mobile-only Plan Builder view: shows one day at a time (a 7-day tab switcher) with every person × selected-meal-type as a vertically stacked `AssignmentCell`, so there is no horizontal scrolling on a phone. Reuses `isCellAvailable`/`findCellAssignments` from `lib/calculations.ts` unchanged; drives `AssignmentCell`'s `tapEligible` via `canPlaceEntryInCell(selectedEntry, ...)` and is never itself a drag-and-drop target (`droppable` is always `false` — placement here is tap-only). |
